...
1.  **tidyverse: used to make it easier to use and visualize data in R.**

2.  **skimr: used to provide summary statistics for quick and informative data exploration.**

3.  **ggplot: used to create data visualization in the form of graphs.**

4.  **read_csv(): used to read data from a CSV file.**

5.  **view(): used to open data in the viewer window.**

6.  **glimpse(): used to quickly view data in the first few columns.**

7.  **head(): used to view the first few rows of data.**

8.  **tail(): used to view the last few rows of data.**

9.  **select(): used to choose specific columns of data based on the name.**

10. **filter(): used to subset rows in a data frame.**

11. **arrange(): used to sort rows of data based on one or more columns.**

12. **desc(): used to sort columns in descending order.**

13. **geom_histogram(): used to create a histogram plot.**

14. **geom_point(): used to create a scatterplot.**
...
**Your response here: Yes, I believe that a tibble is represented as classes that includes terms. Also, in the console, it is labeled as a tibble.**
...
**Your response here: There is some data missing from the file.**
...
```{r}
new_object1<-select(sci_classes,-c(student_id,FinalGradeCEMS))
```
...
filtered_students<-filter(sci_classes, percentage_earned<=0.6 & subject=="FrScA")
head(filtered_students)
```
...
sci_classes %>%
  ...
  ggplot() +
  geom_point(aes(x = TimeSpent_hours, y = score, color = Gender), alpha = 0.7) +
  facet_wrap(~subject)
